...
public class UsuariosDAO extends HibernateDaoSupport {
	...
	public void saveOrUpdate(Usuarios transientInstance) {
		log.debug("saving Usuarios instance");
		try {
	...
	public Usuarios findById(java.lang.Long id) {
	...
	public List<Usuarios> findByEmailPassword(Object email,Object password) {
		try {
			String queryString = "from Usuarios as model where model.usremail= ? and model.password=?";
			return getHibernateTemplate().find(queryString, email,password);
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
			throw re;
		}
	}
	public List<Usuarios> findByIdAuxiliar(Object value1,Object value2) {
		try {
			String queryString = "from Usuarios as model where model.usridexterno= ? and model.provider=?";
